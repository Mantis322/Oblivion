...
export default function PathDetailPage() {
  ...
  const handlePurchasePath = async () => {
    ...
    try {
      ...
      if (path.isFree) {
        // Auto-enroll in free path
        const result = await obiService.purchasePath(pathId, user.walletAddress);
        if (result.success) {
          await loadPathData();
        } else {
          alert('Failed to enroll in path. Please try again.');
        }
      } else {
        // For paid paths, you would integrate with payment processing here
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center flex-wrap gap-6 text-sm">
                      <span className="flex items-center text-slate-400">
                        ...
                        <span className="text-purple-400 font-medium">{assets.length}</span> Assets
                      </span>
                      <span className="flex items-center text-slate-400">
                        <Users className="h-4 w-4 mr-2 text-blue-400" />
                        <span className="text-blue-400 font-medium">{enrolledCount}</span> Enrolled
                      </span>
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-400 mt-1">
                    This path is priced at {path.price} XLM. Purchase to access all content!
                  </p>
                </div>
              </div>
            </div>
              ...
              {filteredAssets.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p className="text-slate-400 mb-6 max-w-md mx-auto">
                    {searchTerm ? 'No assets match your search criteria.' : 'This path doesn\'t have any assets yet.'}
                  </p>
                  {isOwner && !searchTerm && (
                    <button
                      onClick={() => router.push(`/obi/path/${pathId}/create-asset`)}
                ...
                <div className="space-y-4">
                  {filteredAssets.map((asset, index) => (
                    <div key={asset.id} className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6 hover:bg-slate-700/50 hover:border-slate-600/50 transition-all duration-200">
