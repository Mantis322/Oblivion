...
export default function PostComponent({ post, onPostDeleted }: PostComponentProps) {
  ...
  }, [showDeleteModal, showCommentModal, showEmojiPicker]);
  ...
  useEffect(() => {
    const fetchBookmarkState = async () => {
      if (currentUser && post.id) {
        // For reposts, check the original post; for regular posts, check the post itself
        const targetPostId = post.isRepost ? post.originalPostId! : post.id;
        const bookmarked = await checkUserBookmarked(targetPostId, currentUser.walletAddress);
  ...
  const handleBookmark = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      const result = await toggleBookmark(targetPostId, currentUser.walletAddress);
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      ...
      // Build commentData without undefined fields
      ...
      // Only add userImg if avatar exists
      if (currentUser.avatar) {
        commentData.userImg = currentUser.avatar;
      }
      ...
      if (success) {
        ...
        // Reload comments to get the actual Firebase data
        await loadComments();
        setCommentText('');
        setSelectedMedia(null);
        setMediaType(null);
    ...
    >
      ...
      {post.isRepost && (
        <div className="flex items-center gap-2 mb-3 text-slate-400 text-sm relative z-10">
          ...
          <span>
            <span className="font-medium text-green-400">{post.userDisplayName}</span> reposted
          </span>
        </div>
      )}
      ...
      <div className="flex gap-4 relative z-10">
        ...
        <div className="flex-shrink-0">
          <div className="relative group/avatar">
            {(post.isRepost ? post.originalUserImg : currentUserAvatar) ? (
              <img 
        ...
        <div className="flex-1 min-w-0">
          {/* Header - Show original post author for reposts */}
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3 min-w-0">
              <h3 className="font-bold text-white truncate hover:text-blue-400 transition-colors duration-200 cursor-pointer">
                {post.isRepost ? (post.originalUserDisplayName || 'User') : post.userDisplayName}
              </h3>
              <span className="text-slate-400 text-sm truncate hover:text-slate-300 transition-colors duration-200 cursor-pointer">
          ...
          {/* Post Text - Show original post text for reposts */}
          <p className="text-slate-100 mb-4 leading-relaxed text-base">
          ...
          <div className="flex items-center justify-between max-w-md">
            ...
            >
              ...
              {isReposting && <span className="ml-2 w-3 h-3 border-2 border-green-400/30 border-t-green-500 rounded-full animate-spin"></span>}
            </button>
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-semibold text-white text-sm">{post.userDisplayName}</span>
      ...
      {showDeleteModal && isClient && createPortal(
        ...
        >
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl shadow-2xl border border-slate-700/50 p-8 max-w-md w-full mx-4 transform transition-all duration-300 scale-100">
            {/* Header */}
